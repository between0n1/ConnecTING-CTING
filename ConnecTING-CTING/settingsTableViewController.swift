...
//        var currentUser = PFUser.current() should be global?
    ...
    func logout_displayAlert() { // will be used when user succefully created an account.
        let alert = UIAlertController(title: "Logout", message: "Do you want to logout?", preferredStyle: .alert)
        let okAction = UIAlertAction(title: "Sign Out", style: .default, handler: {action in self.logout()})
